...
#ifndef __OPT_API_H__
#define __OPT_API_H__
...
/// \brief Dynamic library import and export definition
	...
	#ifdef OPT_API_DLL_BUILD
		#define OPT_API _declspec(dllexport)
	#else
		#define OPT_API _declspec(dllimport)
	#endif

	#define OPT_CALL __stdcall
#else
	#define OPT_API
	#define OPT_CALL
#endif
...
/// \return Success, return OPT_OK. Failure, return error code 
/// \remarks
...
OPT_API int OPT_CALL OPT_EnumDevices(OUT OPT_DeviceList *pDeviceList, IN unsigned int interfaceType);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_EnumDevicesByUnicast(OUT OPT_DeviceList *pDeviceList, IN const char* pIpAddress);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_CreateHandle(OUT OPT_HANDLE* handle, IN OPT_ECreateHandleMode mode, IN void* pIdentifier);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_DestroyHandle(IN OPT_HANDLE handle);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetDeviceInfo(IN OPT_HANDLE handle, OUT OPT_DeviceInfo *pDevInfo);
...
/// \param handle [IN] Device handle
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_Open(IN OPT_HANDLE handle);

/// \~chinese
/// \brief  打开设备
/// \param handle [IN] 设备句柄
/// \param accessPermission [IN] 控制通道权限(OPT_Open默认使用accessPermissionControl权限)
/// \return 成功，返回OPT_OK；错误，返回错误码
/// \~english
/// \brief  Open Device
/// \param handle [IN] Device handle
/// \param accessPermission [IN] Control access permission(Default used accessPermissionControl in OPT_Open)
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_OpenEx(IN OPT_HANDLE handle, IN OPT_ECameraAccessPermission accessPermission);
...
OPT_API bool OPT_CALL OPT_IsOpen(IN OPT_HANDLE handle);
...
/// \param handle [IN] Device handle
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_Close(IN OPT_HANDLE handle);
...
OPT_API int OPT_CALL OPT_GIGE_ForceIpAddress(IN OPT_HANDLE handle, IN const char* pIpAddress, IN const char* pSubnetMask, IN const char* pGateway);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GIGE_GetAccessPermission(IN OPT_HANDLE handle, OUT OPT_ECameraAccessPermission* pAccessPermission);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GIGE_SetAnswerTimeout(IN OPT_HANDLE handle, IN unsigned int timeout);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_DownLoadGenICamXML(IN OPT_HANDLE handle, IN const char* pFullFileName);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_SaveDeviceCfg(IN OPT_HANDLE handle, IN const char* pFullFileName);

/// \~chinese
/// \brief 从文件加载设备xml配置
/// \param handle [IN] 设备句柄
/// \param pFullFileName [IN] 设备配置(xml)文件全名(含路径)，如：D:\\config.xml 或 D:\\config.mvcfg
/// \param pErrorList [OUT] 加载失败的属性名列表。存放加载失败的属性上限为OPT_MAX_ERROR_LIST_NUM。
/// \return 成功，返回OPT_OK；错误，返回错误码
/// \~english 
/// \brief load the configuration of the device
/// \param handle [IN] Device handle
/// \param pFullFileName [IN] The full path name of the property file(xml). e.g. D:\\config.xml or D:\\config.mvcfg
/// \param pErrorList [OUT] The list of load failed properties. The failed to load properties list up to OPT_MAX_ERROR_LIST_NUM.
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_LoadDeviceCfg(IN OPT_HANDLE handle, IN const char* pFullFileName, OUT OPT_ErrorList* pErrorList);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_WriteUserPrivateData(IN OPT_HANDLE handle, IN void* pBuffer, IN_OUT unsigned int* pLength);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_ReadUserPrivateData(IN OPT_HANDLE handle, OUT void* pBuffer, IN_OUT unsigned int* pLength);
...
/// \param pLength [IN] Byte count written expected   [OUT] Byte count written in fact
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_WriteUARTData(IN OPT_HANDLE handle, IN void* pBuffer, IN_OUT unsigned int* pLength);
...
/// \param pLength [IN] Byte count read expected   [OUT] Byte count read in fact
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_ReadUARTData(IN OPT_HANDLE handle, OUT void* pBuffer, IN_OUT unsigned int* pLength);
...
OPT_API int OPT_CALL OPT_SubscribeConnectArg(IN OPT_HANDLE handle, IN OPT_ConnectCallBack proc, IN void* pUser);
...
/// Only one call back function is supported.\n
/// Registration becomes invalid after the device is closed, , and need to re-register after the device is opened
OPT_API int OPT_CALL OPT_SubscribeParamUpdateArg(IN OPT_HANDLE handle, IN OPT_ParamUpdateCallBack proc, IN void* pUser);
...
/// Only one call back function is supported.\n
/// Registration becomes invalid after the device is closed, , and need to re-register after the device is opened
OPT_API int OPT_CALL OPT_SubscribeStreamArg(IN OPT_HANDLE handle, IN OPT_StreamCallBack proc, IN void* pUser);
...
/// Only one call back function is supported.\n
/// Registration becomes invalid after the device is closed, , and need to re-register after the device is opened
OPT_API int OPT_CALL OPT_SubscribeMsgChannelArg(IN OPT_HANDLE handle, IN OPT_MsgChannelCallBack proc, IN void* pUser);
...
OPT_API int OPT_CALL OPT_SetBufferCount(IN OPT_HANDLE handle, IN unsigned int nSize);
...
/// \param handle [IN] Device handle
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_ClearFrameBuffer(IN OPT_HANDLE handle);
...
OPT_API int OPT_CALL OPT_GIGE_SetInterPacketTimeout(IN OPT_HANDLE handle, IN unsigned int nTimeout);
...
OPT_API int OPT_CALL OPT_GIGE_SetSingleResendMaxPacketNum(IN OPT_HANDLE handle, IN unsigned int maxPacketNum);
...
OPT_API int OPT_CALL OPT_GIGE_SetMaxLostPacketNum(IN OPT_HANDLE handle, IN unsigned int maxLostPacketNum);
...
OPT_API int OPT_CALL OPT_USB_SetUrbTransfer(IN OPT_HANDLE handle, IN unsigned int nNum, IN unsigned int nSize);
...
/// \param handle [IN] Device handle
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_StartGrabbing(IN OPT_HANDLE handle);

/// \~chinese
/// \brief 开始取流
/// \param handle [IN] 设备句柄
/// \param maxImagesGrabbed [IN] 允许最多的取帧数，达到指定取帧数后停止取流，如果为0，表示忽略此参数连续取流(OPT_StartGrabbing默认0)
/// \param strategy [IN] 取流策略,(OPT_StartGrabbing默认使用grabStrartegySequential策略取流)
/// \return 成功，返回OPT_OK；错误，返回错误码
/// \~english
/// \brief Start grabbing
/// \param handle [IN] Device handle
/// \param maxImagesGrabbed [IN] Maximum images allowed to grab, once it reaches the limit then stop grabbing; 
/// If it is 0, then ignore this parameter and start grabbing continuously(default 0 in OPT_StartGrabbing)
/// \param strategy [IN] Image grabbing strategy; (Default grabStrartegySequential in OPT_StartGrabbing)
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_StartGrabbingEx(IN OPT_HANDLE handle, IN uint64_t maxImagesGrabbed, IN OPT_EGrabStrategy strategy);
...
OPT_API bool OPT_CALL OPT_IsGrabbing(IN OPT_HANDLE handle);
...
/// \param handle [IN] Device handle
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_StopGrabbing(IN OPT_HANDLE handle);
...
/// \return 成功，返回OPT_OK；错误，返回错误码
/// \remarks
/// 该异步获取帧数据机制和同步获取帧数据机制(OPT_GetFrame)互斥，对于同一设备，系统中两者只能选其一\n
/// 只支持一个回调函数, 且设备关闭后，注册会失效，打开设备后需重新注册
/// \~english
...
/// \return Success, return OPT_OK. Failure, return error code
/// \remarks
/// This asynchronous getting frame data mechanism and synchronous getting frame data mechanism(OPT_GetFrame) are mutually exclusive,\n
/// only one method can be choosed between these two in system for the same device.\n
/// Only one call back function is supported.\n
/// Registration becomes invalid after the device is closed, , and need to re-register after the device is opened
OPT_API int OPT_CALL OPT_AttachGrabbing(IN OPT_HANDLE handle, IN OPT_FrameCallBack proc, IN void* pUser);
...
/// \remarks
/// 该接口不支持多线程调用。\n
/// 该同步获取帧机制和异步获取帧机制(OPT_AttachGrabbing)互斥,对于同一设备，系统中两者只能选其一。\n
/// 使用内部缓存获取图像，需要OPT_ReleaseFrame进行释放图像缓存。
/// \~english
...
/// This synchronous getting frame data mechanism and asynchronous getting frame data mechanism(OPT_AttachGrabbing) are mutually exclusive,\n
/// only one method can be chose between these two in system for the same device.\n
/// Use internal cache to get image, need to release image buffer by OPT_ReleaseFrame
OPT_API int OPT_CALL OPT_GetFrame(IN OPT_HANDLE handle, OUT OPT_Frame* pFrame, IN unsigned int timeoutMS);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_ReleaseFrame(IN OPT_HANDLE handle, IN OPT_Frame* pFrame);
...
/// \return Success, return OPT_OK. Failure, return error code 
/// \remarks
/// Use OPT_ReleaseFrame to free image buffer

OPT_API int OPT_CALL OPT_CloneFrame(IN OPT_HANDLE handle, IN OPT_Frame* pFrame, OUT OPT_Frame* pCloneFrame);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetChunkDataByIndex(IN OPT_HANDLE handle, IN OPT_Frame* pFrame, IN unsigned int index, OUT OPT_ChunkDataInfo *pChunkDataInfo);

/// \~chinese
/// \brief 获取流统计信息(OPT_StartGrabbing / OPT_StartGrabbing执行后调用)
/// \param handle [IN] 设备句柄
/// \param pStreamStatsInfo [OUT] 流统计信息数据
/// \return 成功，返回OPT_OK；错误，返回错误码
/// \~english
/// \brief Get stream statistics infomation(Used after excuting OPT_StartGrabbing / OPT_StartGrabbing)
/// \param handle [IN] Device handle
/// \param pStreamStatsInfo [OUT] Stream statistics infomation
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetStatisticsInfo(IN OPT_HANDLE handle, OUT OPT_StreamStatisticsInfo* pStreamStatsInfo);

/// \~chinese
/// \brief 重置流统计信息(OPT_StartGrabbing / OPT_StartGrabbing执行后调用)
/// \param handle [IN] 设备句柄
/// \return 成功，返回OPT_OK；错误，返回错误码
/// \~english
/// \brief Reset stream statistics infomation(Used after excuting OPT_StartGrabbing / OPT_StartGrabbing)
/// \param handle [IN] Device handle
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_ResetStatisticsInfo(IN OPT_HANDLE handle);
...
OPT_API bool OPT_CALL OPT_FeatureIsAvailable(IN OPT_HANDLE handle, IN const char* pFeatureName);
...
OPT_API bool OPT_CALL OPT_FeatureIsReadable(IN OPT_HANDLE handle, IN const char* pFeatureName);
...
OPT_API bool OPT_CALL OPT_FeatureIsWriteable(IN OPT_HANDLE handle, IN const char* pFeatureName);
...
OPT_API bool OPT_CALL OPT_FeatureIsStreamable(IN OPT_HANDLE handle, IN const char* pFeatureName);
...
OPT_API bool OPT_CALL OPT_FeatureIsValid(IN OPT_HANDLE handle, IN const char* pFeatureName);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetIntFeatureValue(IN OPT_HANDLE handle, IN const char* pFeatureName, OUT int64_t* pIntValue);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetIntFeatureMin(IN OPT_HANDLE handle, IN const char* pFeatureName, OUT int64_t* pIntValue);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetIntFeatureMax(IN OPT_HANDLE handle, IN const char* pFeatureName, OUT int64_t* pIntValue);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetIntFeatureInc(IN OPT_HANDLE handle, IN const char* pFeatureName, OUT int64_t* pIntValue);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_SetIntFeatureValue(IN OPT_HANDLE handle, IN const char* pFeatureName, IN int64_t intValue);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetDoubleFeatureValue(IN OPT_HANDLE handle, IN const char* pFeatureName, OUT double* pDoubleValue);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetDoubleFeatureMin(IN OPT_HANDLE handle, IN const char* pFeatureName, OUT double* pDoubleValue);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetDoubleFeatureMax(IN OPT_HANDLE handle, IN const char* pFeatureName, OUT double* pDoubleValue);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_SetDoubleFeatureValue(IN OPT_HANDLE handle, IN const char* pFeatureName, IN double doubleValue);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetBoolFeatureValue(IN OPT_HANDLE handle, IN const char* pFeatureName, OUT bool* pBoolValue);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_SetBoolFeatureValue(IN OPT_HANDLE handle, IN const char* pFeatureName, IN bool boolValue);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetEnumFeatureValue(IN OPT_HANDLE handle, IN const char* pFeatureName, OUT uint64_t* pEnumValue);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_SetEnumFeatureValue(IN OPT_HANDLE handle, IN const char* pFeatureName, IN uint64_t enumValue);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetEnumFeatureSymbol(IN OPT_HANDLE handle, IN const char* pFeatureName, OUT OPT_String* pEnumSymbol);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_SetEnumFeatureSymbol(IN OPT_HANDLE handle, IN const char* pFeatureName, IN const char* pEnumSymbol);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetEnumFeatureEntryNum(IN OPT_HANDLE handle, IN const char* pFeatureName, OUT unsigned int* pEntryNum);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetEnumFeatureEntrys(IN OPT_HANDLE handle, IN const char* pFeatureName, IN_OUT OPT_EnumEntryList* pEnumEntryList);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_GetStringFeatureValue(IN OPT_HANDLE handle, IN const char* pFeatureName, OUT OPT_String* pStringValue);
...
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_SetStringFeatureValue(IN OPT_HANDLE handle, IN const char* pFeatureName, IN const char* pStringValue);
...
/// \param pFeatureName [IN] Feature name
/// \return Success, return OPT_OK. Failure, return error code 
OPT_API int OPT_CALL OPT_ExecuteCommandFeature(IN OPT_HANDLE handle, IN const char* pFeatureName);
...
OPT_API int OPT_CALL OPT_PixelConvert(IN OPT_HANDLE handle, IN_OUT OPT_PixelConvertParam* pstPixelConvertParam);
...
/// \return Success, return OPT_OK. Failure, return error code
OPT_API int OPT_CALL OPT_OpenRecord(IN OPT_HANDLE handle, IN OPT_RecordParam *pstRecordParam);
...
/// \return Success, return OPT_OK. Failure, return error code
OPT_API int OPT_CALL OPT_InputOneFrame(IN OPT_HANDLE handle, IN OPT_RecordFrameInfoParam *pstRecordFrameInfoParam);
...
/// \param handle [IN] Device handle
/// \return Success, return OPT_OK. Failure, return error code
OPT_API int OPT_CALL OPT_CloseRecord(IN OPT_HANDLE handle);
...
OPT_API int OPT_CALL OPT_FlipImage(IN OPT_HANDLE handle, IN_OUT OPT_FlipImageParam* pstFlipImageParam);
...
OPT_API int OPT_CALL OPT_RotateImage(IN OPT_HANDLE handle, IN_OUT OPT_RotateImageParam* pstRotateImageParam);
